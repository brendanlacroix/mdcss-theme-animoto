examples.lang = {
  color: function (pre, value) {
    ...
    colors.className = 'colors text-center';
    ...
    function parseColor(color) {
      ...
      swatchNode.className = 'color-swatch label body-text-sm';
      ...
      swatchNode.style.textShadow = '0 0 1px ' + (contrastColor === '#ffffff' ? '#282828' : '#ffffff');
      ...
      Object.keys(color).filter(function (key) { return key !== 'color'; }).forEach(function (key) {
        ...
        propertyNode.className = 'color-property body-text';

        propertyNode.setAttribute('data-name', key);

        var preNode = document.createElement('pre');

        preNode.innerHTML = '$' + color[key];
        preNode.className = 'inverted';

        propertyNode.appendChild(preNode);
      });
